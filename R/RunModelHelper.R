...
creatDataList_One = function(data,control_coverage,data_coverage){
  nSNP = dim(data)[1]
  z = data$Alt_D
...
creatDataList_Two = function(data,control_coverage,data_coverage){
  nSNP = dim(data)[1]
  z = data$Alt_D
  N = data$Alt_D+data$Ref_D
  Nclust = 3
...
creatDataList_LOH = function(data,control_coverage,data_coverage){
  nSNP = dim(data)[1]
  z = data$Alt_D
  N = data$Alt_D+data$Ref_D
  Nclust = 4
...
creatDataList_Four = function(data,control_coverage,data_coverage){
  nSNP = dim(data)[1]
  z = data$Alt_D
  N = data$Alt_D+data$Ref_D
  Nclust = 5
...
runNormal = function(data,data_coverage,control_coverage,checkConvergence,
                     adapt,burnin,nChain,nStep,thinSteps){
  ...
  codaSamples_one = coda.samples(jagsModel_one, variable.names=parameters_one, 
...
runMonosomy = function(data,data_coverage,control_coverage,checkConvergence,
                       adapt,burnin,nChain,nStep,thinSteps){
...
runMitoticTrisomy = function(data,data_coverage,control_coverage,checkConvergence,
                             adapt,burnin,nChain,nStep,thinSteps){
...
runMeioticTrisomy = function(data,data_coverage,control_coverage,checkConvergence,
                             adapt,burnin,nChain,nStep,thinSteps){
...
runLOH = function(data,data_coverage,control_coverage,checkConvergence,
                  adapt,burnin,nChain,nStep,thinSteps){
...
################# Likelihood and BIC ################
# calculate log maximum likelihood for ONEMIX model using posterior distribution
log_likelihood_one = function(posterior,dataList){
  ...
  for (i in 1:length(z)){
    sub_loglike =
      log(p1*dbetabinom.ab(z[i],N[i],a,b)+p2*dbetabinom.ab(z[i],N[i],1,1))
  ...
  for (i in 1:length(N_cov)){
    sub_log_cov = dnbinom(N_cov[i],size=r_cov,prob=p_cov,log=T)
...
# calculate log maximum likelihood for TWOMIX model using posterior distribution
log_likelihood_two = function(posterior,dataList){
  parameter = apply(posterior,2,median)
  z = dataList$z
  N = dataList$N
  ...
  for (i in 1:length(z)){
    sub_loglike =
      log(p1*dbetabinom.ab(z[i],N[i],a1,b1)+p2*dbetabinom.ab(z[i],N[i],a2,b2)+p3*dbetabinom.ab(z[i],N[i],1,1))
    loglike_aaf = loglike_aaf + sub_loglike
  }
  #print(loglike_aaf)
...
# calculate log maximum likelihood for FOURMIX model using posterior distribution
log_likelihood_four = function(posterior,dataList){
  parameter = apply(posterior,2,median)
  z = dataList$z
  N = dataList$N
  a1 = parameter["mu[1]"]*parameter["kappa"]
  ...
  for (i in 1:length(z)){
    sub_loglike =
      log(p1*dbetabinom.ab(z[i],N[i],a1,b1)+p2*dbetabinom.ab(z[i],N[i],a2,b2)+p3*dbetabinom.ab(z[i],N[i],a3,b3)+
            p4*dbetabinom.ab(z[i],N[i],a4,b4)+p5*dbetabinom.ab(z[i],N[i],1,1))
    loglike_aaf = loglike_aaf + sub_loglike
  }
  #print(loglike_aaf)
...
log_likelihood_UPD = function(posterior,dataList){
  parameter = apply(posterior,2,median)
  z = dataList$z
  N = dataList$N
  a1 = parameter["mu[1]"]*parameter["kappa"]
...
calculate_BIC = function(posterior,dataList,type="two"){
