## function to prepare GC content and average coverage for each targeted region
...
prepareCoverageGC = function(target_bed, bam, genome_assembly="hg19"){
  ...
  if(length(gc) == length(target_gr)){
    mcols(target_gr)$GC = gc
                  ...
                  length(gc), "GC content calculated. Please check your input.",
                  sep=" "))
  target_gr
}
...
#' @param ... additional \code{GRanges} object to pass. \strong{Note1:} If there is 
#' only one Granges object given, then coverage will be corrected by GC content.
...
#' sequenced by the same protocols, which means the targeted region is the same
...
#' @param plot \code{Boolean} Default: \code{TRUE}. If \code{TRUE}, the coverage 
#' vs. GC content plot before and after normalization will be plotted. And the 
#' average coverage for each chromosome before and after normalization will be 
#' plotted
...
#' sequencing. Nature Genetics, 41(10):1061-7.
#' @importFrom preprocessCore normalize.quantiles
#' @export
normalizeCoverage = function(object,...,control=NULL,
                             writeToFile=T,destination=NULL,plot=T){
  if(is.null(control)) {
    ...
    sample_name = c(control_name,as.character(substitute(list(object,...)))[-1])
  }
  nSample = length(data)
  ...
  if (nSample > 1) {
    ...
    corrected = NULL
    for (i in 1:nSample){
      message(paste("correct GC bias in sample '",sample_name[i],"' ...",sep=""))
    ...
    message(paste("correct GC bias in sample '",sample_name[1],"' ...",sep=""))
  ...
  if(nSample>1){
    ...
    if(is.null(control_name)){
      ...
      ## take median of normed average coverage for each chromosome as reference
      ref_cov = apply(after_chr,2,median)
    }
    ## if control is provided, take average coverage for control as reference
  ...
  ## if write to file requested, then write the normalized coverage table into
...
#' @import IRanges
#' @export
prepareHetero = function(vcffile,target_bed,genome="hg19",
                         writeToFile=T,destination=NULL){
